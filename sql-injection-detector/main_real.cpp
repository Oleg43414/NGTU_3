...
public:
    ...
    static bool initDB(const QString& host = "localhost", int port = 5432, 
                      const QString& dbname = "sqli_detector", 
                      const QString& user = "postgres", const QString& pass = "password") {
        ...
        // Создаем таблицы
        QSqlQuery query;
        query.exec("CREATE TABLE IF NOT EXISTS sqli_attacks ("
                   "id SERIAL PRIMARY KEY, attack_number INTEGER UNIQUE NOT NULL, "
                   "ip VARCHAR(45), site VARCHAR(255), url TEXT, payload TEXT, "
                   "type VARCHAR(50), response_time INTEGER, status_code INTEGER, "
                   "timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP)");
        
        query.exec("CREATE TABLE IF NOT EXISTS targets ("
                   "id SERIAL PRIMARY KEY, domain VARCHAR(255) UNIQUE, "
                   "scan_count INTEGER DEFAULT 0, last_scanned TIMESTAMP)");
    ...
    static bool addAttack(const AttackRecord& record) {
        QSqlQuery query;
        query.prepare("INSERT INTO sqli_attacks (attack_number, ip, site, url, payload, type, response_time, status_code) "
...
public:
    MainWindow(QWidget *parent = nullptr) : QMainWindow(parent) {
        ...
        startTimer();
        resize(1600, 900);
        setWindowTitle("REAL SQLi DETECTOR v5.0 - PostgreSQL");
    }
...
private:
    void setupUI() {
        ...
        title = new QLabel("🚨 REAL SQLi DETECTOR v5.0 - PostgreSQL + LIVE SCAN");
        ...
        targetInput->setPlaceholderText("http://testphp.vulnweb.com или ваш разрешенный сайт");
        ...
        clearBtn = new QPushButton("🗑️ CLEAR DB");
        ...
        statusLabel = new QLabel("🚨 Подключение к PostgreSQL... | Атак: 0");
        ...
        connect(quickBtn, &QPushButton::clicked, this, &MainWindow::quickAttacks);
    ...
    void setupDatabase() {
        if (SQLiDetector::initDB("localhost", 5432, "sqli_detector", "postgres", "password")) {
            statusLabel->setText("✅ PostgreSQL подключен | Атак: 0");
        } else {
            statusLabel->setText("❌ PostgreSQL недоступен!");
            QMessageBox::warning(this, "DB Error", "PostgreSQL недоступен!");
        }
    }
    
    void startTimer() {
        uiTimer = new QTimer(this);
        connect(uiTimer, &QTimer::timeout, this, &MainWindow::updateTable);
        uiTimer->start(1000);
    }
    ...
    int scanSite(const QString& baseUrl, const QString& payload) {
        ...
        int time = reply->attribute(QNetworkRequest::HttpPipeliningWasUsedAttribute).toInt();
    ...
    void scanTarget() {
        ...
        QString ip = "127.0.0.1"; // TODO: реальный IP
    ...
    void clearAll() {
        ...
        statusLabel->setText("🟢 DB Очищена!");
    }
    ...
    void exportCSV() {
        ...
        statusLabel->setText("💾 CSV экспортирован!");
    }
    ...
    void updateTable() {
        ...
        statusLabel->setText(QString("🚨 PostgreSQL | Атак: %1").arg(SQLiDetector::getTotalAttacks()));
    }
